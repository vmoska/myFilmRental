...
public class ApiExceptionValidationHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> handleConstraintViolationException(ConstraintViolationException e,
        ...
        List<ApiFieldError> apiFieldError = ex
                .stream().map(fieldError -> new ApiFieldError(fieldError.getPropertyPath().toString(),
                        "Constraint violation", fieldError.getMessage(), fieldError.getInvalidValue()))
                .collect(Collectors.toList());

        return new ResponseEntity<Object>(new ApiResponseCustom(Instant.now(), 422, "Bad Request", apiFieldError,
                request.getDescription(false).replace("uri=", "")), HttpStatus.UNPROCESSABLE_ENTITY);
    }
    ...
    protected ResponseEntity<Object> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException e,
        ...
        ApiFieldError api = new ApiFieldError(e.getName(), "Mysmatch type", e.getMessage(),
                request.getParameter(e.getName()));

        return new ResponseEntity<Object>(new ApiResponseCustom(Instant.now(), 422, "Bad Request", api,
                request.getDescription(false).replace("uri=", "")), HttpStatus.UNPROCESSABLE_ENTITY);
    }
